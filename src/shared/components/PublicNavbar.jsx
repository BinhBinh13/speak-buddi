...
export default function PublicNavbar({
  ...
  // When not scrolled and not app route, navbar is transparent.
  // HeroSection has light background, so text should be dark by default.
  // Only switch to light text if the section behind is dark — handled by scrolled state.
  const isDarkBg = scrolled || isApp;
  ...
  return (
    <nav
      style={{
        ...
        background: isDarkBg ? "rgba(250,250,247,0.92)" : "transparent",
        backdropFilter: isDarkBg ? "blur(12px)" : "none",
        borderBottom: isDarkBg ? `1px solid ${COLORS.creamDark}` : "none",
    ...
    >
      <div style={{
        ...
        <Logo isDark={!isDarkBg} />
        {isApp ? <AppNav pathname={pathname} /> : <PublicLinks isDark={!isDarkBg} />}
        {isApp ? <AppRight user={user} /> : <PublicCTA isDark={!isDarkBg} />}
      </div>
    </nav>
  );
}
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
      ...
      <button onClick={() => setNotifOpen((v) => !v)} style={{
        ...
        🔔
        <span style={{ position: "absolute", top: 6, right: 6, width: 7, height: 7, borderRadius: "50%", background: COLORS.coral, border: "1.5px solid white" }} />
      </button>
      <div style={{
        width: 34, height: 34, borderRadius: 10,
        ...
        display: "flex", alignItems: "center", justifyContent: "center",
        fontFamily: FONTS.body, fontSize: 13, fontWeight: 700, color: COLORS.emeraldDark, cursor: "pointer",
      }}>{user.name[0]}</div>
    </div>
  );
}

function PublicLinks({ isDark }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 36 }}>
      {PUBLIC_NAV_LINKS.map((item) => (
        <a key={item} href="#" style={{
          fontFamily: FONTS.body, fontSize: 14.5,
          color: isDark ? "rgba(255,255,255,0.85)" : COLORS.navyMid,
          textDecoration: "none", opacity: 0.85, transition: "opacity 0.2s, color 0.3s",
        }}
...
function PublicCTA({ isDark }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      <a href="/login" style={{
        fontFamily: FONTS.body, fontSize: 14,
        color: isDark ? "rgba(255,255,255,0.85)" : COLORS.navy,
        textDecoration: "none", padding: "8px 18px",
        transition: "color 0.3s",
      }}>Log in</a>
      <button onClick={goToSpeaking} style={{
        fontFamily: FONTS.body, fontSize: 14, fontWeight: 600,
        background: isDark ? "white" : COLORS.navy,
        color: isDark ? COLORS.navy : "white",
        border: "none", borderRadius: 10,
        padding: "10px 22px", cursor: "pointer",
        transition: "transform 0.15s, background 0.2s, color 0.2s",
      }}
        onMouseEnter={(e) => { e.currentTarget.style.background = COLORS.emeraldDark; e.currentTarget.style.color = "white"; e.currentTarget.style.transform = "translateY(-1px)"; }}
        onMouseLeave={(e) => { e.currentTarget.style.background = isDark ? "white" : COLORS.navy; e.currentTarget.style.color = isDark ? COLORS.navy : "white"; e.currentTarget.style.transform = "translateY(0)"; }}
